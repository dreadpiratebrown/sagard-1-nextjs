  ...
  devtools(
    persist(
      (set, get) => ({
        ...
        updateStore: (payload, sectionId = null) => {
          const state = get();
          const updates = { ...payload };

          if (sectionId && state.lastAppliedSection === sectionId) {
            return {}; // no changes
          }
          ...
          if (payload.xp) {
            ...
            updates.xp = newXp;
            if (newXp >= 20 && state.level < 3) {
              updates.level = 3;
            }
            if (newXp >= 60 && state.level < 4) {
              updates.level = 4;
            }
            if (newXp >= 150 && state.level < 5) {
              updates.level = 5;
            }
          }
          ...
          // Merge/remove/update logic for trophies
          if (payload.trophies) {
            let updatedTrophies = [...state.trophies];

            payload.trophies.forEach((incoming) => {
              // Case 1: explicit remove
              if (incoming.remove || incoming.name?.startsWith("-")) {
                const name = incoming.name.startsWith("-")
                  ? incoming.name.slice(1)
                  : incoming.name;
                updatedTrophies = updatedTrophies.filter(
                  (t) => t.name !== name
                );
                return;
              }

              // Case 2: already exists → merge properties
              const index = updatedTrophies.findIndex(
                (t) => t.name === incoming.name
              );
              if (index >= 0) {
                updatedTrophies[index] = {
                  ...updatedTrophies[index],
                  ...incoming,
                };
              } else if (incoming.add) {
                // Case 3: new trophy → add
                updatedTrophies.push(incoming);
              }
            });

            updates.trophies = updatedTrophies;
          }

          if (payload.lostToKnight) {
            const newValue = state.lostToKnight + payload.lostToKnight;
            updates.lostToKnight = newValue;
          }

          if (payload.menInWhite) {
            const newValue = state.menInWhite + payload.menInWhite;
            updates.menInWhite = newValue;
          }

          if (sectionId) {
            updates.lastAppliedSection = sectionId;
          }
